...
import { Avatar, Box, Button, Card, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, Typography } from '@mui/material';
...
interface TeamMembersSectionProps {
  ...
  renderFieldValue: (field: any) => React.ReactNode;
}

export default function TeamMembersSection({ memberGroups, colors, participantEmail, renderFieldValue }: TeamMembersSectionProps) {
  const [selectedMemberGroup, setSelectedMemberGroup] = useState<any>(null);
    ...
    <Box sx={{ mb: 5 }}>
      ...
      <Grid container spacing={3}>
        {memberGroups.map((group, gIdx) => {
          const nameField = group.fields.find((f: any) => f.label?.toLowerCase().includes("name") && !f.label?.toLowerCase().includes("father") && !f.label?.toLowerCase().includes("mother") && !f.label?.toLowerCase().includes("school"));
          const emailField = group.fields.find((f: any) => f.label?.toLowerCase().includes("email") && !f.label?.toLowerCase().includes("father") && !f.label?.toLowerCase().includes("mother") && !f.label?.toLowerCase().includes("principal") && !f.label?.toLowerCase().includes("school"));
          
          let emailToDisplay = emailField?.value || "";
          ...
          return (
            <Grid size={{ xs: 12, sm: 6, md: 4 }} key={gIdx}>
              <Card sx={{ p: 3, borderRadius: 4, border: `1px solid ${colors.BORDER}`, boxShadow: "0 10px 30px -10px rgba(0,0,0,0.05)", display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', height: '100%', position: 'relative', overflow: 'visible' }}>
                <Typography variant="caption" sx={{ color: colors.PRIMARY, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, mb: 2 }}>
                  {group.title}
                </Typography>
                <Avatar sx={{ width: 80, height: 80, mb: 2, bgcolor: "rgba(99, 102, 241, 0.1)", color: colors.PRIMARY }}>
                  <Person sx={{ fontSize: 40 }} />
                </Avatar>
                <Typography variant="h6" sx={{ fontWeight: 700, color: colors.TEXT_PRIMARY, mb: 0.5 }}>
                  {nameField ? nameField.value : "Unknown Name"}
                </Typography>
                <Typography variant="body2" sx={{ color: colors.TEXT_SECONDARY, mb: 3 }}>
                  {emailToDisplay || "No Email Provided"}
                </Typography>
                <Box sx={{ mt: 'auto', width: '100%' }}>
                  <Button fullWidth variant="outlined" onClick={() => setSelectedMemberGroup(group)} sx={{ borderRadius: 2, textTransform: 'none', fontWeight: 600, borderColor: colors.PRIMARY, color: colors.PRIMARY, '&:hover': { bgcolor: "rgba(99, 102, 241, 0.04)" } }}>
                    View Details
                  </Button>
                </Box>
              </Card>
