    ...
    >
      ...
      >
        {(() => {
          ...
          if (thumbnailUrl) {
            return <img src={thumbnailUrl} alt="Thumbnail" style={{ width: "100%", height: "100%", objectFit: "cover" }} />;
          }
      ...
      <Box sx={{ width: { xs: "100%", md: "65%" }, p: { xs: 3, md: 4 }, display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2, flexWrap: "wrap", gap: 2 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, flex: 1, alignItems: 'flex-start' }}>
            {(() => {
              ...
              let category = "";
              const categoryField = entryFields?.find((f: any) => f.label?.toLowerCase().includes("category"));
        ...
        >
          ...
          <Box component="span" sx={{ color: colors.TEXT_PRIMARY, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 1 }}>
            {(() => {
            ...
            {(() => {
              const sData = entry?.submission?.data || {};
              const entryFields = entry?.contest?.entryLevelTemplate?.schema?.fields || entry?.contest?.entry_level_template?.schema?.fields || [];
              const participationTypeField = entryFields.find((f: any) => f.label?.toLowerCase().includes("participation type"));
              ...
              if (!type) {
                // fallback for the payload specifically provided
